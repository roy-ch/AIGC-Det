...
from IMDLBenCo.datasets import AIGCDetectionDataset
...
def get_args_parser():
    ...
    parser.add_argument('--is_DRCT', action='store_true',
                        help='Use DRCT training data if specified.')
    # ----DRCT数据集相关
    parser.add_argument('--prob_aug', default=0.5, type=float, help="Setting the probability for augmentation")
    parser.add_argument('--prob_cutmixup_real_fake', default=0.4, type=float,
                        help="Probability for performing cutmix between real and fake images")
    parser.add_argument('--prob_cutmixup_real_rec', default=0.3, type=float,
                        help="Probability for performing cutmix between real and reconstructed images")
    parser.add_argument('--prob_cutmixup_real_real', default=0.3, type=float,
                        help="Probability for performing cutmix between two real images")
    ...
    parser.add_argument("--fake_indexes", default='1', help="Setting the fake indexes, multi class using '1,2,3,...' ",
                        type=str)
...
def main(args, model_args):
    ...
    if not args.is_DRCT:
        ...
        dataset_train = AIGCDetectionDataset(root_path=args.root_path, fake_root_path=args.fake_root_path, fake_indexes=args.fake_indexes,
                                   ...
                                   prob_cutmixup_real_fake=args.prob_cutmixup_real_fake,
                                   prob_cutmixup_real_rec=args.prob_cutmixup_real_rec,
                                   prob_cutmixup_real_real=args.prob_cutmixup_real_real,
                                   is_padding=args.if_padding,
                                   is_resizing=args.if_resizing,
                                   output_size=(args.image_size, args.image_size),
                                   common_transforms=test_transform,
