...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # logits = F.interpolate(logits, size=(256, 256), mode="bilinear", align_corners=False) # 对齐形状
        
        outputs = {}
        outputs["mask"] = torch.flatten(logits, start_dim=1)
        guided_feature = feature * torch.sigmoid(logits)
        
        label_logits = self.conv_cls(guided_feature)
        outputs["logit"] = torch.flatten(label_logits, start_dim=1).squeeze()
